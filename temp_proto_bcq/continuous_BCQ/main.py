...
def train_BCQ(state_dim, action_dim, max_action, device, args):
    ...
    # Initialize policy
    policy = BCQ2.BCQ2(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi)
...
# Trains DDPG offline
def train_DDPG(state_dim, action_dim, max_action, device, args):
    # For saving files
    setting = f"{args.env}_{args.seed}"
    buffer_name = f"{args.buffer_name}_{setting}"

    # Initialize policy
    policy = DDPG2.DDPG2(state_dim, action_dim, max_action, device, args.discount, args.tau)
    ...
    while training_iters < args.max_timesteps:
        ...
        evaluations.append(eval_policy(policy, args.env, args.seed))
        np.save(f"./results/DDPG_{setting}", evaluations)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_algo", default=None)
    ...
    if args.train_behavioral:
        ...
        print(f"Setting: Training in Batch mode: {args.batch_algo}, Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")
    ...
    if args.train_behavioral or args.generate_buffer:
        ...
        if args.batch_algo == 'bcq':
            train_BCQ(state_dim, action_dim, max_action, device, args)
        elif args.batch_algo == 'ddpg':
            train_DDPG(state_dim, action_dim, max_action, device, args)
        else:
