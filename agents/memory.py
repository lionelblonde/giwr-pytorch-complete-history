...
class ReplayBuffer(object):
    ...
    def __init__(self, capacity, shapes, save_full_history):
        ...
        if self.save_full_history:
            self.full_history = {n: [] for n in self.shapes.keys()}
    ...
    def append(self, transition):
        ...
        for k in self.ring_buffers.keys():
            self.ring_buffers[k].append(transition[k])  # append from the RingBuffer class
        if self.save_full_history:
            for k in self.full_history.keys():
                self.full_history[k].append(transition[k])  # append from the List class

    def load(self, to_load_in_memory):
