...
class DDPGAgent(object):
    ...
    def __init__(self, env, device, hps, to_load_in_memory):
        ...
        self.actr = Actor(self.env, self.hps, self.rms_obs, hidden_dims[0]).to(self.device)
        ...
        if self.param_noise is not None:
            ...
            self.pnp_actr = Actor(self.env, self.hps, self.rms_obs, hidden_dims[0]).to(self.device)
            self.pnp_actr.load_state_dict(self.actr.state_dict())
            # Create adaptive-parameter-noise-perturbed ('apnp') actor
            self.apnp_actr = Actor(self.env, self.hps, self.rms_obs, hidden_dims[0]).to(self.device)
